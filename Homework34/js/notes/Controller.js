class Controller {
    ...
    initialize() {
        document.addEventListener('DOMContentLoaded', () => {
            ...
            this.#view.forms.addEventListener('submit', this.#buttonModalHandler);
    ...
    #allButtonsTaskHandler = (e) => {
        ...
        if (e.target.closest('[data-mark-btn]')){
            this.#model.toggleImportant(usedElement);
            this.#view.toggleImportant(usedElement);
        }
        if (e.target.closest('[data-unmark-btn]')){
            this.#model.unToggleImportant(usedElement);
            this.#view.unToggleImportant(usedElement);
        }
        if(e.target.closest('[data-addNote-btn]')){
            const id = Number(usedElement.getAttribute('data-id'));
            const lol = JSON.parse(localStorage.getItem('example'))[id-1];
            this.#view.createModelTemplate(lol)
            this.#view.modalOpen();
        }
        return false;
    }
